...
async function loadOrderSummary() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      orderItemsContainer.innerHTML = '<div class="text-center">Error loading order</div>';
    }
  } catch (error) {
    orderItemsContainer.innerHTML = '<div class="text-center">Network error</div>';
  }
}
...
function setupPaymentMethods() {
  ...
  paymentOptions.forEach(option => {
    option.addEventListener('click', function() {
      ...
      if (method === 'card') {
        alert('Credit/Debit card payment coming soon! Please use PayPal for now.');
        // Revert to PayPal
  ...
  try {
    ...
    if (paymentResult.success) {
      ...
      if (response.ok) {
        ...
        await fetch(`${API_BASE_URL}/cart`, getFetchOptions({
          method: 'DELETE'
        }));
