...
async function loadProductDetails() {
  ...
  try {
    container.innerHTML =
      '<div class="loading"><h3>Loading product details...</h3></div>';
    ...
    if (response.ok) {
      ...
      container.innerHTML =
        '<div class="error"><h3>Product not found</h3></div>';
    }
  } catch (error) {
    container.innerHTML =
      '<div class="error"><h3>Error loading product</h3></div>';
  }
}
          ...
          ${
            ...
            <div class="quantity-selector">
              ...
              <div class="quantity-controls">
                ...
                <input type="number" class="quantity-input" id="quantityInput" value="${quantity}" min="1" max="${
                  product.stock
...
async function removeFromCart() {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/cart/${currentProduct.id}`,
      getFetchOptions({
        method: "DELETE",
      })
    );
...
async function toggleFavorite() {
  ...
  try {
    if (isFavorite) {
      const response = await fetch(
        `${API_BASE_URL}/favorites/${currentProduct.id}`,
        getFetchOptions({
          method: "DELETE",
        })
      );
